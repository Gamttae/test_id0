...
(function () {
  ...
  const CONFIG = {
    studentId: '20262094',
    userId: 'lift.roves_0p',
    flipRevealStudentIdPartial: 5,   // shows first 4 chars: "2026"
    flipRevealStudentIdFull: 15,     // shows full: "20262094"
    flipRevealUserId: 60,
    flipRevealInsta: 15,             // show insta icon at 15 flips
  ...
  let studentIdPartialRevealed = false;
  ...
  const $bracketTransition = document.getElementById('bracket-transition');
  ...
  // ═══════════════════════════════════════════════════════════
  // BRACKET TRANSITION SEQUENCE
  // Loading content fades + brackets converge to center → expand to main scene
  // ═══════════════════════════════════════════════════════════
  function startBracketTransition() {
    const $lbTL = document.querySelector('.lb-tl');
    const $lbBR = document.querySelector('.lb-br');
    const $transTL = $bracketTransition.querySelector('.trans-tl');
    const $transBR = $bracketTransition.querySelector('.trans-br');

    const vh = window.innerHeight;
    const vw = window.innerWidth;
    const centerX = Math.round(vw / 2 - 2);
    const centerY = Math.round(vh / 2 - 2);

    // Final positions for main scene brackets (same as .bracket-tl / .bracket-br)
    const finalTL = { top: 60, left: 20, size: 30 };
    const finalBR = { top: vh - 60 - 30, left: vw - 20 - 30, size: 30 };

    // Step 1: Animate loading brackets to center while content fades
    if ($lbTL) {
      $lbTL.style.top = centerY + 'px';
      $lbTL.style.left = centerX + 'px';
      $lbTL.style.width = '4px';
      $lbTL.style.height = '4px';
    }
    if ($lbBR) {
      $lbBR.style.bottom = 'auto';
      $lbBR.style.right = 'auto';
      $lbBR.style.top = centerY + 'px';
      $lbBR.style.left = centerX + 'px';
      $lbBR.style.width = '4px';
      $lbBR.style.height = '4px';
    }

    // Fade out loading text content simultaneously
    ...
    // Step 2: After brackets have converged, switch to transition overlay
    setTimeout(() => {
      $loading.style.display = 'none';

      // Place transition brackets at center (no transition yet)
      $bracketTransition.style.display = 'block';
      $transTL.style.top = centerY + 'px';
      $transTL.style.left = centerX + 'px';
      $transTL.style.width = '4px';
      $transTL.style.height = '4px';

      $transBR.style.top = centerY + 'px';
      $transBR.style.left = centerX + 'px';
      $transBR.style.width = '4px';
      $transBR.style.height = '4px';

      // Force reflow
      void $transTL.offsetHeight;

      // Enable transition and move to final positions
      $transTL.classList.add('animating');
      $transBR.classList.add('animating');

      // Use requestAnimationFrame to ensure transition fires
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          $transTL.style.top = finalTL.top + 'px';
          $transTL.style.left = finalTL.left + 'px';
          $transTL.style.width = finalTL.size + 'px';
          $transTL.style.height = finalTL.size + 'px';

          $transBR.style.top = finalBR.top + 'px';
          $transBR.style.left = finalBR.left + 'px';
          $transBR.style.width = finalBR.size + 'px';
          $transBR.style.height = finalBR.size + 'px';
        });
      });

      // Step 3: After expansion, fade overlay and show main scene
      setTimeout(() => {
        $bracketTransition.style.transition = 'opacity 0.5s ease';
        $bracketTransition.style.opacity = '0';
        ...
        setTimeout(() => {
          $bracketTransition.style.display = 'none';
        }, 600);
      }, 800);
    }, 650);
  }
  ...
  // Number of visible (locked) characters in student ID during partial reveal
  let studentIdLockedChars = 0;
  ...
  function startScrambleAnimation() {
    ...
    function loop() {
      if (!studentIdRevealed) {
        const len = CONFIG.studentId.length;
        const locked = CONFIG.studentId.substring(0, studentIdLockedChars);
        const rest = randomScramble(len - studentIdLockedChars);
        $studentIdDisplay.textContent = locked + rest;
      }
      if (!userIdRevealed) {
  ...
  // Partial typing: reveal characters one by one up to `count`, rest stays scrambled
  function partialTypeReveal(count) {
    $studentIdDisplay.classList.remove('scrambling');
    $studentIdDisplay.classList.add('partial-revealed');
    let idx = studentIdLockedChars; // start from current locked position
    const interval = setInterval(() => {
      if (idx >= count) {
        clearInterval(interval);
        studentIdLockedChars = count;
        return;
      }
      studentIdLockedChars = idx + 1;
      idx++;
    }, 80);
  }

  function checkReveal() {
    // Stage 1: partial student ID (first 4 chars "2026") at 5 flips — type one by one
    if (!studentIdPartialRevealed && flipCount >= CONFIG.flipRevealStudentIdPartial) {
      studentIdPartialRevealed = true;
      setTimeout(() => partialTypeReveal(4), 400);
    }
    // Stage 2: full student ID at 15 flips — type remaining chars one by one then mark revealed
    if (!studentIdRevealed && flipCount >= CONFIG.flipRevealStudentIdFull) {
      studentIdRevealed = true;
      $studentIdDisplay.classList.remove('partial-revealed');
      // Type from wherever we are (4) to full length
      let idx = studentIdLockedChars;
      const fullLen = CONFIG.studentId.length;
      const interval = setInterval(() => {
        if (idx >= fullLen) {
          clearInterval(interval);
          studentIdLockedChars = fullLen;
          $studentIdDisplay.classList.remove('scrambling');
          $studentIdDisplay.classList.add('revealed');
          $studentIdDisplay.textContent = CONFIG.studentId;
          return;
        }
        studentIdLockedChars = idx + 1;
        idx++;
    ...
    // Insta icon reveal at 15 flips
    if (!instaRevealed && flipCount >= CONFIG.flipRevealInsta) {
      instaRevealed = true;
      $instaIcon.style.display = '';
      $instaIcon.classList.add('visible');
    }
    // User ID at 60 flips
