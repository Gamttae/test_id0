...
(function () {
  ...
  function startBracketTransition() {
    ...
    // Move loading brackets to screen center (converge to a dot)
    if ($lbTL) {
      $lbTL.style.top = 'calc(50% - 3px)';
      $lbTL.style.left = 'calc(50% - 3px)';
      $lbTL.style.width = '6px';
      $lbTL.style.height = '6px';
      $lbTL.style.opacity = '1';
    }
    if ($lbBR) {
      $lbBR.style.bottom = 'auto';
      $lbBR.style.right = 'auto';
      $lbBR.style.top = 'calc(50% - 3px)';
      $lbBR.style.left = 'calc(50% - 3px)';
      $lbBR.style.width = '6px';
      $lbBR.style.height = '6px';
      $lbBR.style.opacity = '1';
    }
    ...
    // Step 2: After brackets have converged (0.6s), switch to transition overlay
    setTimeout(() => {
      $loading.style.display = 'none';

      // Show bracket transition overlay with brackets at center
      $bracketTransition.style.display = 'block';
      $transTL.classList.add('phase-center');
      $transBR.classList.add('phase-center');

      // Force reflow then expand to main scene positions
      void $transTL.offsetHeight;

      setTimeout(() => {
        $transTL.classList.remove('phase-center');
        $transBR.classList.remove('phase-center');
        $transTL.classList.add('phase-final');
        $transBR.classList.add('phase-final');
  ...
  function startScrambleAnimation() {
    ...
    function loop() {
      if (!studentIdRevealed) {
        if (studentIdPartialRevealed) {
          // Show first 4 chars, scramble the rest
          const visible = CONFIG.studentId.substring(0, 4);
          const rest = randomScramble(CONFIG.studentId.length - 4);
          $studentIdDisplay.textContent = visible + rest;
        } else {
  ...
  function checkReveal() {
    // Stage 1: partial student ID (first 4 chars "2026") at 5 flips
    ...
    // Stage 2: full student ID at 15 flips
