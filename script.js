...
(function () {
  ...
  function drawParticles() {
    ...
    // Subtle vignette
    const vigGrad = bgCtx.createRadialGradient(w/2, h/2, w*0.2, w/2, h/2, w*0.8);
    ...
    for (const s of floatingShapes) {
      ...
      bgCtx.strokeStyle = `rgba(212, 168, 83, ${s.alpha})`;
      bgCtx.lineWidth = 0.5;
      bgCtx.beginPath();
    ...
    for (let i = 0; i < particles.length; i++) {
      for (let j = i + 1; j < particles.length; j++) {
        ...
        if (dist < 80) {
          ...
          bgCtx.strokeStyle = `rgba(212, 168, 83, ${0.025 * (1 - dist / 80)})`;
          bgCtx.lineWidth = 0.3;
          bgCtx.stroke();
        }
      }
    ...
    if (Math.random() < 0.003) {
      ...
      grad.addColorStop(0.5, 'rgba(212, 168, 83, 0.04)');
      grad.addColorStop(0.7, 'rgba(212, 168, 83, 0.02)');
  ...
  // ═══════════════════════════════════════════════════════════
  // 4. 3D CARD INTERACTION — directional flip with lock
  // ═══════════════════════════════════════════════════════════
  let isFlipping = false; // lock during flip animation
  ...
  document.addEventListener('touchend', (e) => {
    ...
    if (absDx > 50 && absDx > absDy * 1.2) {
      // dx < 0 means finger moved LEFT → card rotates LEFT (negative Y)
      // dx > 0 means finger moved RIGHT → card rotates RIGHT (positive Y)
