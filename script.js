...
(function () {
  ...
  function drawParticles() {
    ...
    // Draw floating geometric shapes — more visible on light bg
    for (const s of floatingShapes) {
      ...
      bgCtx.strokeStyle = `rgba(184, 146, 62, ${s.alpha * 2.5})`;
      bgCtx.lineWidth = 0.6;
      bgCtx.beginPath();
    ...
    for (let i = 0; i < particles.length; i++) {
      for (let j = i + 1; j < particles.length; j++) {
        ...
        if (dist < 80) {
          ...
          bgCtx.strokeStyle = `rgba(184, 146, 62, ${0.06 * (1 - dist / 80)})`;
          bgCtx.lineWidth = 0.4;
          bgCtx.stroke();
        }
      }
    ...
    for (const p of particles) {
      ...
      bgCtx.fillStyle = `hsla(${p.hue}, ${p.sat}%, 45%, ${p.alpha * 1.5})`;
      bgCtx.fill();
    }
    ...
    if (Math.random() < 0.003) {
      ...
      grad.addColorStop(0.3, 'rgba(184, 146, 62, 0.04)');
      grad.addColorStop(0.5, 'rgba(184, 146, 62, 0.07)');
      grad.addColorStop(0.7, 'rgba(184, 146, 62, 0.04)');
  ...
  function resetCardFloat() {
    ...
    // Force a reflow so the CSS var is applied before animation restarts
    void $card.offsetHeight;
    $card.style.transform = '';
    $card.style.animation = '';
  }

  // Listen for transition end on the card to unlock
  ...
  function flipCard(direction) {
    if (isFlipping) return; // double-guard
    isFlipping = true;
    ...
    // Safety fallback — if transitionend doesn't fire (rare edge case)
    setTimeout(() => {
      if (isFlipping) {
        isFlipping = false;
        isTouching = false;
  ...
  function applyGlitch() {
    ...
    // Noise pixels — warm tones on light bg
    for (let i = 0; i < 60; i++) {
      ...
      if (Math.random() < 0.5) {
        // Amber noise
        data[idx] = 180 + Math.floor(Math.random() * 50);
        data[idx + 1] = 150 + Math.floor(Math.random() * 40);
        data[idx + 2] = 80 + Math.floor(Math.random() * 40);
      } else {
        // Light/dark noise
  ...
  function startBarcode() {
    ...
    while (x < w - 4) {
      ...
      if (isFilled) {
        ...
        ctx.fillStyle = `rgba(184, 146, 62, ${alpha})`;
